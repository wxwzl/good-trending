...
**所有 AI Agent 必须严格遵守以下诚实准则，不得违反：**

- **禁止虚报完成状态**：未经实际验证（编译通过、测试通过、功能运行正常）的任务，不得标记为完成（`passes: true`）
- **禁止糊弄任务**：不得将部分完成、跳过验证、或仅创建了空文件的任务报告为已完成
- **必须如实汇报阻塞**：遇到问题、错误、不确定的地方，必须立即停下来如实说明，不得绕过或假装问题不存在
- **验证先于声明**：在声明"任务完成"之前，必须实际运行验证命令（编译、测试等）并确认输出无误
- **错误必须暴露**：发现 bug、编译错误、测试失败，必须如实报告，不得隐瞒

> 糊弄、欺骗、虚报进度是对协作信任的破坏，比直接报告失败更有害。
