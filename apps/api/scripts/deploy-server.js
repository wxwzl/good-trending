...
// 获取部署目录
const deployDir =
  process.env.DEPLOY_DIR || path.resolve(__dirname, '../../../app/api');
...
log(
  'INFO',
  `[deploy-server] DATABASE_URL=${process.env.DATABASE_URL ? '已配置' : '未设置'}`,
);
log(
  'INFO',
  `[deploy-server] REDIS_URL=${process.env.REDIS_URL ? '已配置' : '未设置'}`,
);
