...
const logger = createSchedulerLogger("product-discovery");
...
export async function processProductDiscoveryJob(
  ...
  try {
    const [{ saveCrawledProducts, saveCrawlerLog, GoogleSearchCrawler }, categoryList] =
      await Promise.all([importCrawler(), getAllCategories()]);

    if (categoryList.length === 0) {
      logger.warn("No categories found, skipping product discovery");
    ...
    // 执行商品发现爬取
    const crawlResult = (await crawler.crawlProductsByCategory(categoryList)) as CrawlResult;
