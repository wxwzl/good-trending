...
-- 10. 创建触发器函数：自动创建未来分区
CREATE OR REPLACE FUNCTION auto_create_future_partitions()
RETURNS TRIGGER AS $$
DECLARE
    next_month DATE;
    next_year INT;
    next_month_num INT;
BEGIN
    next_month := DATE_TRUNC('month', NEW.stat_date) + INTERVAL '1 month';
    next_year := EXTRACT(YEAR FROM next_month)::INT;
    next_month_num := EXTRACT(MONTH FROM next_month)::INT;

    PERFORM create_monthly_partition('product_social_stat', next_year, next_month_num);

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 创建触发器（在插入时自动创建下个月的分区）
CREATE TRIGGER trg_auto_create_partition
    BEFORE INSERT ON product_social_stat
    FOR EACH ROW
    EXECUTE FUNCTION auto_create_future_partitions();
