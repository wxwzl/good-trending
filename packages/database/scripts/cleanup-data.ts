...
import { trendRanks, productSocialStats, categoryHeatStats, crawlerLogs } from "../src/schema";
...
async function cleanupData() {
  ...
  try {
    ...
    const dailyRankResult = await db
      .delete(trendRanks)
      .where(and(eq(trendRanks.periodType, "TODAY"), lt(trendRanks.statDate, ninetyDaysAgo)));
    ...
    const yesterdayRankResult = await db
      .delete(trendRanks)
      .where(and(eq(trendRanks.periodType, "YESTERDAY"), lt(trendRanks.statDate, ninetyDaysAgo)));
    ...
    const midTermTypes = ["THIS_WEEK", "LAST_7_DAYS", "LAST_15_DAYS", "LAST_30_DAYS"];

    for (const periodType of midTermTypes) {
      const result = await db
        .delete(trendRanks)
        .where(and(eq(trendRanks.periodType, periodType), lt(trendRanks.statDate, oneYearAgo)));
