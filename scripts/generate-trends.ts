...
async function generateTrends(): Promise<void> {
  ...
  // 1. 检查商品数量
  const productCount = await db.select({ count: db.fn.count() }).from(products);
  ...
  // 4. 显示生成的趋势数据
  const trendCount = await db.select({ count: db.fn.count() }).from(trends);
  console.log(`\n📊 今天生成的趋势记录数: ${trendCount[0]?.count || 0}`);
